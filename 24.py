...
# Menghitung aktor boundary spanner
boundary_spanners = []
for node in G.nodes():
    neighbors = set(G.neighbors(node))
    neighbor_degrees = [degree_centrality[n] for n in neighbors]
    if len(neighbors) >= 2 and sum(neighbor_degrees) < 1:
        boundary_spanners.append((node, degree_centrality[node]))

# Mengambil kolom sentiment_label dari sentimen_data berdasarkan aktor-aktor yang ditemukan
actor_sentiments = sentiment_data[sentiment_data['author'].str.lower().isin([node[0].lower() for node in top_10_creators + top_10_influencers + boundary_spanners])][['author', 'sentiment_label']]
...
# Menampilkan aktor boundary spanner beserta sentiment label
boundary_spanner_results = []
for spanner, degree in boundary_spanners:
    sentiment_label = actor_sentiments.loc[actor_sentiments['author'].str.lower() == spanner.lower(), 'sentiment_label'].values[0]
    boundary_spanner_results.append({'Boundary Spanner': spanner, 'Degree': degree, 'Sentiment Label': sentiment_label})
...
with pd.ExcelWriter("social_analysis_results_new.xlsx") as writer:
    ...
    boundary_spanner_results_df = pd.DataFrame(boundary_spanner_results)
    boundary_spanner_results_df.to_excel(writer, sheet_name='Boundary Spanners', index=False)

print("Output As: ", 'social_analysis_results_new.xlsx')